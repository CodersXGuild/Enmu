...
from EnmuBot.utils.pluginhelper import runcmd


async def convert_to_image(event, borg):
    Uff = await event.get_reply_message()
    if not (
        Uff.gif
        or Uff.audio
        or Uff.voice
    ...
    if not os.path.exists(downloaded_file_name):
        ...
        await lel.delete
        return
    if Uff and Uff.photo:
        Uff_final = downloaded_file_name
    elif Uff.sticker and Uff.sticker.mime_type == "application/x-tgsticker":
        ...
        os.remove(rpath)
        Uff_final = image_name20
    elif Uff.sticker and Uff.sticker.mime_type == "image/webp":
        ...
        if not os.path.exists(imgpath):
            ...
            return
        Uff_final = imgpath
    elif Uff.gif or Uff.video or Uff.video_note:
