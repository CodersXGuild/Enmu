...
async def _get_married_users(chat_id: int) -> Dict[int, int]:
    married_users = await coupledb.find_one({"chat_id": chat_id})
    if not married_users:
        return {}
    return married_users["married_users"]


async def get_married_users(chat_id: int) -> List[int]:
    married_users = await _get_married_users(chat_id)
    return list(married_users.keys())


async def get_marriage_status(chat_id: int, user_id: int) -> Union[bool, int]:
    married_users = await _get_married_users(chat_id)
    if user_id in married_users:
        return married_users[user_id]
    return False


async def marry_users(chat_id: int, user1_id: int, user2_id: int):
    married_users = await _get_married_users(chat_id)
    married_users[user1_id] = user2_id
    married_users[user2_id] = user1_id
    await coupledb.update_one(
        {"chat_id": chat_id},
        {"$set": {"married_users": married_users}},
        upsert=True,
    )


async def divorce_users(chat_id: int, user1_id: int, user2_id: int) -> bool:
    married_users = await _get_married_users(chat_id)
    if (
        user1_id in married_users
        and user2_id in married_users
        and married_users[user1_id] == user2_id
        and married_users[user2_id] == user1_id
    ):
        del married_users[user1_id]
        del married_users[user2_id]
        await coupledb.update_one(
            {"chat_id": chat_id},
            {"$set": {"married_users": married_users}},
            upsert=True,
