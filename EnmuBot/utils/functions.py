...
from EnmuBot import aiohttpsession as aiosession
from EnmuBot.ex_plugins.dbfunctions import start_restart_stage
from EnmuBot.utils.http import get, post


async def restart(m: Message):
    if m:
        await start_restart_stage(m.chat.id, m.message_id)
    execvp(executable, [executable, "-m", "EnmuBot"])
...
def generate_captcha():
    ...
    height = 100
    correct_answer = ""
    font = ImageFont.truetype("EnmuBot/resources/default.ttf", 55)
    file = f"EnmuBot/resources/{randint(1000, 9999)}.jpg"
