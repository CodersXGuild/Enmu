...
import telegram
from EnmuBot import TIGERS, WOLVES, dispatcher
...
from EnmuBot.modules.helper_funcs.extraction import (
    extract_text,
    extract_user,
    extract_user_and_text,
)
from EnmuBot.modules.helper_funcs.filters import CustomFilters
from EnmuBot.modules.helper_funcs.misc import split_message
from EnmuBot.modules.helper_funcs.string_handling import split_quotes
from EnmuBot.modules.log_channel import loggable
from EnmuBot.modules.sql import warns_sql as sql
...
@user_admin_no_reply
# @user_can_ban Turn on if u want to spoil the admin ryts XD
@bot_admin
@loggable
...
def __stats__():
    return (
        f"𒊹 {sql.num_warns()} overall warns, across {sql.num_warn_chats()} chats.\n"
        f"𒊹 {sql.num_warn_filters()} warn filters, across {sql.num_warn_filter_chats()} chats."
    )
