import os
from EnmuBot.modules.sql.night_mode_sql import (
    add_nightmode,
    rmnightmode,
    get_all_chat_id,
    is_nightmode_indb,
...
from EnmuBot.events import register
from EnmuBot import telethn as tbot, OWNER_ID
    ...
    for pro in chats:
        try:
            await tbot.send_message(
              int(pro.chat_id), "It's 11:59 Pm. \nGroup Is Closing Till 05:59 Am. \nNight Mode Started !"
            )
            await tbot(
...
async def job_open():
    ...
    for pro in chats:
        try:
            await tbot.send_message(
              int(pro.chat_id), "It's 05:59 Am, Group Is Opening."
            )
            await tbot(
            functions.messages.EditChatDefaultBannedRightsRequest(
...
*Admins only Commands*:
  »  /NightMode *:* On / Off to on or off
  
  -| ⚠ Please Note ⚠ |-
  Please Note Time zone for closing and opening of group 
  is Aisa/Kolkata so closing and opening time will be 
  11:59 Ist to 5:59 Ist.
"""
