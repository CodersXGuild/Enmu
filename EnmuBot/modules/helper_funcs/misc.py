...
def clomse_btn(update: Update, context: CallbackContext) -> str:
    bot = context.bot
    ...
    if len(pairs) > 99:
        pairs = pairs[modulo_page * 3:3 * (modulo_page + 1)] + [
            (EqInlineKeyboardButton("⬅️", callback_data="{}_prev({})".format(prefix, modulo_page)),
                EqInlineKeyboardButton("Back", callback_data="clomse_btn"),
             EqInlineKeyboardButton("➡️", callback_data="{}_next({})".format(prefix, modulo_page)))]

    else:
        pairs += [[EqInlineKeyboardButton("Close", callback_data="clomse_btn")]]
...
CLOSE_BUTTON_HANDLER = CallbackQueryHandler(clomse_btn, pattern=r"clomse_")
dispatcher.add_handler(CLOSE_BUTTON_HANDLER)
__handlers__ = [CLOSE_BUTTON_HANDLER]
