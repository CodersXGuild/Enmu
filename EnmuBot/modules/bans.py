...
import EnmuBot.modules.sql.users_sql as sql
from EnmuBot.modules.disable import DisableAbleCommandHandler
from EnmuBot.modules.helper_funcs.filters import CustomFilters
from EnmuBot import (
    DEV_USERS,
    LOGGER,
    OWNER_ID,
...
from EnmuBot.modules.helper_funcs.chat_status import (
    user_admin_no_reply,
    bot_admin,
    can_restrict,
    connection_status,
...
from EnmuBot.modules.helper_funcs.extraction import extract_user_and_text
from EnmuBot.modules.helper_funcs.string_handling import extract_time
from EnmuBot.modules.log_channel import gloggable, loggable
...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    try:
        ...
        return log_message
    if user_id == bot.id:
        message.reply_text("Like I'll Do That.")
        return log_message

    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            message.reply_text("Trying to Ban My owner Huh U want to die?")
            ...
            message.reply_text(
                "Bring an order from My Master to fight a XLower Moon."
            )
            ...
            message.reply_text("Can't Ban Admin.")
        return log_message
...
def temp_ban(update: Update, context: CallbackContext) -> str:
    ...
    if not user_id:
        message.reply_text("Can't Find the user.")
        return log_message
    ...
    try:
        ...
        if excp.message != "User not found":
            raise
        message.reply_text("I Am Not Able To Find This Person.")
        return log_message
    if user_id == bot.id:
...
def punch(update: Update, context: CallbackContext) -> str:
    ...
    try:
        ...
        message.reply_text("I Am Not Able To Find This Human.")
        return log_message
    if user_id == bot.id:
...
def punchme(update: Update, context: CallbackContext):
    ...
    if res:
        ...
        update.effective_message.reply_text("Huh? What I can't :/")
...
def unban(update: Update, context: CallbackContext) -> Optional[str]:
    ...
    try:
        ...
        if excp.message != "User not found":
            raise
        message.reply_text("I Am Unable To Find This User.")
        return log_message
    if user_id == bot.id:
    ...
    chat.unban_member(user_id)
    message.reply_text(
        f"{member.user.first_name} [<code>{member.user.id}</code>] Unbanned.",
        parse_mode=ParseMode.HTML
    )
...
def banme(update: Update, context: CallbackContext):
    ...
    user = update.effective_user
    if is_user_admin(update.effective_chat, user_id):
        update.effective_message.reply_text("I Can't Ban A Admin.")
        return
