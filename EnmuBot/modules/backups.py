...
import EnmuBot.modules.sql.notes_sql as sql
from EnmuBot import dispatcher, LOGGER, OWNER_ID, JOIN_LOGGER, SUPPORT_CHAT
from EnmuBot.__main__ import DATA_IMPORT
...
import EnmuBot.modules.sql.rules_sql as rulessql

import EnmuBot.modules.sql.blacklist_sql as blacklistsql
from EnmuBot.modules.sql import disable_sql as disabledsql

import EnmuBot.modules.sql.locks_sql as locksql
...
def export_data(update, context):
    ...
    with open("EnmuBot{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("EnmuBot{}.backup".format(chat_id), "rb"),
        caption="*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `EnmuBot-Backup` was specially made for notes.".format(
            chat.title,
            chat_id,
            tgl,
        ),
    ...
    os.remove("EnmuBot{}.backup".format(chat_id))  # Cleaning file
